...
async function erc721Transfer(
  ...
  const chain = chainId === 100
    ? (await import('viem/chains')).gnosis
  ...
  const rpc = chainId === 100 ? GNOSIS_RPC : BASE_RPC;
...
function TransferModal({ item, onClose }: { item: TransferState; onClose: () => void }) {
  ...
  const explorer = item.chainId === 100 ? 'https://gnosisscan.io/tx/' : 'https://basescan.org/tx/';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl border border-[var(--border)] bg-[#0f0703] shadow-2xl">
        <div className="border-b border-[var(--border)] px-6 py-4">
          ...
          <p className="mt-0.5 text-[10px] text-[var(--muted)]">{item.label} · {item.chainId === 100 ? 'Gnosis' : 'Base'}</p>
        </div>
