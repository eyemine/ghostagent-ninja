...
export function AppNav() {
  ...
  return (
    <>
      {/* ── Desktop: top bar ── */}
      <header className="hidden md:flex fixed top-0 left-0 right-0 z-40 h-14 items-center justify-between border-b border-[var(--border)] bg-[var(--background)]/90 backdrop-blur-md px-8">
      ...
      <header className="md:hidden fixed top-0 left-0 right-0 z-40 border-b border-[var(--border)] bg-[var(--background)]/95 backdrop-blur-md">
        ...
        {mobileOpen && (
          <nav className="flex flex-col border-t border-[var(--border)] bg-[var(--background)] px-4 py-3 gap-1">
            {NAV.map((item) => {
              const active = isActive(item.href);
              return (
                <Link
